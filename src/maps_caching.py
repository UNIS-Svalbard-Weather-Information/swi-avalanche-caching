...
class MapsCaching:
                    ...
                    for root, _, files in os.walk(temp_dir):
                        for file in files:
                            if file.endswith(".tif") or file.endswith(".txt"):
                                ...
                                self.logger.info(
                                    f"DEM file moved and renamed to {new_name}"
                                )

            self.logger.info(
                f"DEM downloaded and extracted to {os.path.join(self.path, 'managed')}"
            )
    ...
    def _compute_steepness_raster(self, res="DTM50"):
        ...
        self.logger.info(
            f"Starting computation of steepness raster from {self.DEM_path}"
        )
        ...
        try:
            with rasterio.open(self.DEM_path) as src:
                ...
                steepness_path = os.path.join(
                    self.path, "managed", f"{res}_steepness_raster.tif"
                )
                with rasterio.open(
                    steepness_path,
                    "w",
                ...
                self.logger.info(
                    f"Steepness raster computed and saved to {steepness_path}"
                )
    ...
    def _create_steepness_contour(
        ...
        try:
            ...
            with rasterio.open(self.steepness_raster_path) as src_steepness:
                ...
                if orientations:
                    with rasterio.open(aspect_path) as src_aspect:
                        ...
                        for orientation in orientations:
                            for min_aspect, max_aspect in ORIENTATION_RANGES[
                                orientation
                            ]:
                                orientation_mask |= (aspect >= min_aspect) & (
                                    aspect < max_aspect
                                )
                ...
                shapes_gen = shapes(
                    mask.astype(np.uint8), mask=mask, transform=src_steepness.transform
                )
                polygons = [
                    shape(geom)
                    for geom, value in tqdm(
                        shapes_gen, desc="Generating polygons", unit="polygon"
                    )
                    if value == 1
                ]

                self.logger.info(
                    f"Generated {len(polygons)} polygons for orientation {orientations}"
                )
                ...
                polygons = [
                    ...
                    for poly in tqdm(
                        polygons, desc="Simplifying polygons", unit="polygon"
                    )
                ]
        ...
        try:
            with rasterio.open(self.DEM_path) as src:
                ...
                aspect_path = os.path.join(
                    self.path, "managed", f"{res}_aspect_raster.tif"
                )
                with rasterio.open(
                    aspect_path,
                    "w",
    ...
    def get_steepness_raster(self, res="DTM50"):
        ...
        if self.force or not self.steepness_raster_path:
            self.logger.info(
                "Steepness raster not found or force flag is set. Computing steepness raster..."
            )
    ...
    def get_aspect_raster(self, res="DTM50"):
        ...
        if self.force or not os.path.exists(aspect_path):
            self.logger.info(
                "Aspect raster not found or force flag is set. Computing aspect raster..."
            )
    ...
    def get_steepness_contour_direction(
        self, orientation, min_steepness=25, max_steepness=55, res="DTM50"
    ):
        """
        Retrieves the file path to the steepness contour shapefile, creating it if necessary, optionally filtered by orientation.
        ...
        self.logger.info(
            f"Checking for existing contour file with orientation at {contour_path}"
        )
        ...
        if self.force or not os.path.exists(contour_path):
            ...
            self.logger.info(
                f"Steepness contour with orientation already exists at {contour_path}"
            )
            self.contour_path = contour_path
