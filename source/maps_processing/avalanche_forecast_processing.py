...
class AvalancheForecastProcessing:
    ...
    def _binary_to_directions(self, binary_string):
        ...
        try:
            ...
            direction_list = [
                directions[i] for i in range(8) if binary_string[i] == "1"
            ]
    ...
    def clip_shapefile_with_gps_contour(self, gps_coordinates, shapefile_path):
        ...
        gdf_polygon["geometry"] = gdf_polygon.buffer(
            10
        ...
        if not intersects.any():
            self.logger.error(
                "The polygon does not intersect with any features in the shapefile."
            )
            return gpd.GeoDataFrame()
        ...
        if clipped_gdf.empty:
            self.logger.warning(
                "The clipped result is empty. Check if the polygon intersects with the shapefile."
            )
    ...
    def fetch_region_data(
        self,
        api_url="https://api01.nve.no/hydrology/forecast/avalanche/v6.3.0/api/Region/A",
    ):
        """
        ...
        try:
            ...
            for region in data:
                ...
                polygon = region["Polygon"][
                    0
                ]  # Assuming polygon is a list with one string element
                ...
                self.regions[region_id] = {"name": name, "polygon": coordinates}
    ...
    def fetch_forecast_data(self):
        ...
        for region_id in self.regions_list:
            ...
            end_day = (today + timedelta(days=self.n_days_forecast)).strftime(
                "%Y-%m-%d"
            )
            ...
            try:
                self.logger.info(
                    f"Fetching forecast data for region {region_id} ({self.regions.get(region_id).get('name', 'Unknown Region')}) from {api_url}"
                )
                response = requests.get(api_url)
                ...
                for forecast in forecast_data:
                    ...
                    if day_key not in self.regions[region_id][
                        "forecast"
                    ] or datetime.fromisoformat(
                        forecast["PublishTime"]
                    ) > datetime.fromisoformat(
                        self.regions[region_id]["forecast"][day_key]["PublishTime"]
                    ):
    ...
    def _create_forecast_layer_region(self, region_info):
        ...
        try:
            ...
            for date, forecast in forecasts.items():
                gdf_dict_list = []
                self.logger.info(
                    f"Processing forecast for date: {forecast['ValidFrom']} & {date}"
                )

                # Correct the condition to check 'MainText' in the current forecast
                if (
                    forecast.get("AvalancheProblems") is None
                    or forecast.get("MainText") == "No Rating"
                ):
                    # Create an empty GeoJSON with available information
                    self.logger.info(
                        f"No detailled AvalancheProblems for region {region_name} and date {date}"
                    )
                    geojson = {
                    ...
                    for problem in forecast["AvalancheProblems"]:
                        ...
                        orientation_list = self._binary_to_directions(
                        ...
                        self.logger.info(
                            f"Orientation list for date = {date} - {orientation_list}"
                        )
                        ...
                        shape_path = self.maps_cache.get_steepness_contour(
                            25,
                            65,
                        ...
                        gdf_dict_list.append(
                            {
                                "gdf": self.clip_shapefile_with_gps_contour(
                                    polygon, shape_path
                                ),
                                "label": label,
                                "description": description,
                ...
                new_date = current_date + timedelta(
                    days=int(date)
                )  # Assuming `date` is a string that can be converted to an integer
    ...
    def _create_forecast_layer_region_2(self, region_info):
        """
        Creates a GeoDataFrame and a dictionary for a region's forecast data.

        This method processes the forecast data for a specified region to generate a GeoDataFrame
        and a dictionary that can be used to create a legend. The GeoDataFrame includes a problem_id
        field that maps to the dictionary containing details for each problem.

        Args:
            region_info (dict): A dictionary containing the region's name, polygon coordinates, and forecast data.

        Returns:
            tuple: A tuple containing a GeoDataFrame and a dictionary. The GeoDataFrame contains the spatial data
                with a 'problem_id' column that maps to the dictionary containing problem details.
        ...
        try:
            ...
            # Initialize a counter for problem_id
            problem_id = 0
            gdf_list = []
            legend_dict = {}
            ...
            for date, forecast in forecasts.items():
                ...
                if (
                    ...
                    self.logger.info(
                        f"No detailed AvalancheProblems for region {region_name} and date {date}"
                    )
                    ...
                    for problem in forecast["AvalancheProblems"]:
                        ...
                        # Add problem_id column
                        gdf["problem_id"] = problem_id

                        # Add to the list
                        gdf_list.append(gdf)

                        # Add to the dictionary
                        legend_dict[problem_id] = {
                            "label": label,
                            "description": description,
                            "date": date,
                        }

                        # Increment the problem_id
                        problem_id += 1
            ...
            if gdf_list:
                ...
                combined_gdf = (
                    gpd.GeoDataFrame()
                )  # Return empty GeoDataFrame if no problems

            return combined_gdf, legend_dict
